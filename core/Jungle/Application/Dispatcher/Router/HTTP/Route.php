...
namespace Jungle\Application\Dispatcher\Router\HTTP {
	 ...
	 */
	class Route extends \Jungle\Application\Dispatcher\Router\Route{
		...
		protected function _beforeMatch(){
			...
			if($this->_preferred_browser && !String::match($request->getClient()->getBrowser()->getName(),$this->_preferred_browser,true)){
				return false;
			}
			...
			if($this->_preferred_sub_domain && !String::match(strstr($request->getHostname(),'.',true),$this->_preferred_content_type,true)){
				return false;
			}
