...
namespace Jungle\Data\Foundation\Record {
	...
	use Jungle\Data\Foundation\Collection\ExtendableInterface;
	use Jungle\Data\Foundation\Collection\ExtendableTrait;
	use Jungle\Data\Foundation\Collection\Sortable\SorterInterface as SortableSorterInterface;
	use Jungle\Data\Foundation\Condition\Condition;
	use Jungle\Data\Foundation\Condition\ConditionBlock;
	...
	use Jungle\Data\Foundation\Record\Collection\Sorter;
	use Jungle\Data\Foundation\Record\Collection\SorterInterface;
	use Jungle\Data\Foundation\Record\Head\Schema;
	use Jungle\Data\Foundation\Registry\RegistryReadInterface;
	use Jungle\Data\Foundation\Schema\OuterInteraction\SchemaAwareInterface;
	...
	class Collection extends
		\Jungle\Data\Foundation\Collection\Enumeration\Collection
		...
		public function getCheckpoint(){
			if(!$this->ancestor){
				...
				if($this->checkpoint && $this->getRoot()->saturation_mode === self::SAT_NONE){
					return $this;
				}else{
		 ...
		 *      является приспособленцем до момента initRecComplete
		 *
		...
		protected function _beforeItemAdd($record){
			if($record->getSchema() !== $this->schema){
				throw new \LogicException('Passed record schema is not support');
			}
		...
		protected function _afterItemAdd($record){
			$this->sorted = false;
			$record->initRecComplete();
			if($this->dirty_capturing && $this->getRoot()->saturation_mode === self::SAT_NONE){
		...
		protected static $_record_assign_function;

		/**
		 * @return \Closure
		 */
		protected function _getAssignFunction(){
			if(!self::$_record_assign_function){
				self::$_record_assign_function = \Closure::bind(function($record, $data){
					/** @var mixed $record Record instance */
					$record->_properties_dirty_applied = true;
					$record->assign($data);
					$record->_properties_dirty_applied = false;
				},$this,Record::class);
			}
			return self::$_record_assign_function;
		}
		...
		public function update($data, $condition = null, $fullSyncIgnoreDirty = false){
			...
			if($level === self::SYNC_STORE){
				...
				if($affected){
					$assign = $this->_getAssignFunction();
