...
namespace Jungle\Data\Foundation {
	...
	abstract class Record
		...
		/** @var  \Jungle\Data\Foundation\Record\Head\Schema */
		protected $_schema;

		protected $_properties_dirty_applied = false;
		...
		/** @var bool */
		protected $_initializing_mode = false;

		/** @var bool */
		protected $_operation_made_execute = false;
		 ...
		 * @return $this
		 */
		public function initRecComplete($operationMade = null){
			if($this->_initializing_mode){
				if($operationMade !== null) $this->setOperationMade($operationMade);
				$this->_schema->markInitialized($this);
				$this->onRecordReady();
				$this->_initializing_mode = false;
			}
			return $this;
		}
		 ...
		 * @return $this
		 */
		public function initRecStart(){
			$this->_initializing_mode = true;
			return $this;
		}
		...
		public function setProperty($key, $value){
			try{
				...
				if($field){
					if(self::$properties_changes_restrict){
						if($field->isReadonly()){
							throw new Exception('Could not set readonly property');
						}
						if($field->isPrivate()){
							throw new Exception('Could not set private property');
						}
					}
					...
					if($this->_properties_dirty_applied){
		...
		public function save(){
			if($this->_operation_made_execute){
			...
			switch($this->_operation_made){
				case self::OP_CREATE:
					if($this->beforeSave() !== false && $this->beforeCreate() !== false){
						$this->_operation_made_execute = true;
						if($this->_doCreate()){
							$this->_operation_made = self::OP_UPDATE;
							$this->_operation_made_execute = false;
							$this->onCreate();
							$this->onSave();
							return true;
						}
		...
		protected function _doCreate(){
			...
			try{
				...
				$this->_afterStorageCreate($data, $pkField->getName(), $store->lastInsertId(),$pkField);
				if($relationFields){
					foreach($relationFields as $field){
