 ...
 * Time: 21:47
 */
namespace Jungle\Data\Storage\Db\Dialect {
	...
	/**
	 * Class MySQL
	 * @package Jungle\Data\Storage\Db\Dialect
	 *
	 *
	 *
	 */
	class MySQL extends Dialect{
		...
		public function createTable(
			...
			foreach($columns as $column){
				$c[] = "\r\n\t" . $this->prepareColumnDefinition(
						...
						isset($column['auto_increment'])?$column['auto_increment']:false
					);
		 ...
		 * @param bool $auto_increment TODO
		 * @return string
		 */
		protected function prepareColumnDefinition(
			$name,$type,$size,$unsigned = false,$notnull = false,$default = null,$zerofill=false, $auto_increment = false
		){
			...
			return $this->escape($name)." {$type}{$size}{$unsigned}{$notnull}{$default}{$zerofill}{$auto_increment}";
		}
		...
		public function renameColumn($table, $columnName, $newName){
			return 'ALTER TABLE '.$this->escapeTableName($this->exportDelimited($table)).' CHANGE COLUMN ' . $this->escape($columnName) . ' TO '. $this->escape($newName). ';';
		}
