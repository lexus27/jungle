...
namespace Jungle\RegExp {
	...
	class Template{
		...
		protected function _compile(){
			...
			// Удовлетворить требования для использования иерархичных маршрутов в приложении
			// Маршрут, если он Контейнер должен компилировать шаблон
			// таким образом чтобы конец не проверялся(проверка без $), возможно отсекая проверенную часть.
			// /users -> @^/users@
			// /users/{id} -> @^/users/\d+@ | @^/\d+@(Вариант, где предварительно необходимо вырезать - то что было проверенно)
			// Оптимизировать для случаев когда
			// Реализовать не-плейсхолдерные сущности которые управляются например {~/users~i} case insensitive.
